...
wp_2023 <- read_csv(here("raw-data", "water potentials", "2022and2023", "2022_2023_alldates_wp.csv")) %>% 
  clean_names() %>% 
  mutate(year = year(ymd(date))) %>% 
  filter(year == 2023) %>% 
  mutate(tree = as.numeric(tag))

wp_2022_1 <- read_csv(here("raw-data", "water potentials",  "2022and2023","2022_2023_alldates_wp.csv")) %>% 
  clean_names() %>% 
  mutate(year = year(ymd(date))) %>% 
  filter(year == 2022)%>% 
  mutate(tree = as.numeric(tag)) 
...
###For general use: 
```{r}
write_csv(wp_all, here("processed-data", "wp_2022_2024.csv"))
```
...
trees_latlong <- bind_rows(pipers_trees_latlong, core_trees_latlong)
  
zenodo_wp_df <- pressure_chamber_wp_df0 %>% 
  select(site,individual_id, species, plot_id, date, pd_md, water_potential_mean, water_potential_sd, water_potential_n) %>% 
  distinct() %>% 
...
write_csv(zenodo_wp_df, here("processed-data", "psinet", "zenodo_wp_df.csv"))

locations_nas <- zenodo_wp_df %>% 
  filter(is.na(latitude)) %>% 
  select(individual_id, site, plot_id, latitude, longitude) %>% 
  distinct()
